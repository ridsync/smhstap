...
class DiagRichStepFirstFragment : BaseFragment() {
    ...
    private fun initFirst(){
        ...
        (activity as DiagRichSurveyActivity).diagnoseInfo.let { diagnoseInfo ->
            when(diagnoseInfo.applyPart){
                DiagRichStepFirstFragment.ADVISOR.ADVISOR_NAME_INVEST.wCode -> {
                    setCheckCardViews(RL_ADVISOR_INVEST)
                    selectedAdvisor = ADVISOR.ADVISOR_NAME_INVEST
                }
                DiagRichStepFirstFragment.ADVISOR.ADVISOR_NAME_MD.wCode -> {
                    setCheckCardViews(RL_ADVISOR_MD)
                    selectedAdvisor = ADVISOR.ADVISOR_NAME_MD
                }
                DiagRichStepFirstFragment.ADVISOR.ADVISOR_NAME_TAX.wCode ->  {
                    setCheckCardViews(RL_ADVISOR_TAX)
                    selectedAdvisor = ADVISOR.ADVISOR_NAME_TAX
                }
                DiagRichStepFirstFragment.ADVISOR.ADVISOR_NAME_HOME_INTE.wCode ->  {
                    setCheckCardViews(RL_ADVISOR_HOME_INTE)
                    selectedAdvisor = ADVISOR.ADVISOR_NAME_HOME_INTE
                }
                DiagRichStepFirstFragment.ADVISOR.ADVISOR_NAME_CM.wCode -> {
                    setCheckCardViews(RL_ADVISOR_CM)
                    selectedAdvisor = ADVISOR.ADVISOR_NAME_CM
                }
                DiagRichStepFirstFragment.ADVISOR.ADVISOR_NAME_MANAGEMENT.wCode -> {
